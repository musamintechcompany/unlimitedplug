    ...
    <form id="assetForm" action="{{ route('admin.digital-assets.update', $digitalAsset) }}" method="POST" enctype="multipart/form-data" x-data="uploadHandler()">
        ...
        <div class="space-y-6">
                    ...
                    <div class="inline-flex flex-col gap-2 min-w-full">
                        ...
                        <div class="flex gap-3">
                            @foreach(config('payment.currencies') as $code => $currency)
                                @php
                                    $pricing = $digitalAsset->prices()->where('currency_code', $code)->first();
                                @endphp
                                <div class="flex-shrink-0 w-32">
                                    <label class="block text-xs font-medium mb-1">{{ $code }} Price *</label>
                                    <input type="number" name="{{ strtolower($code) }}_price" value="{{ old(strtolower($code) . '_price', $pricing->price ?? ($code === 'USD' ? $digitalAsset->price : '')) }}" step="0.01" min="0" required class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded" placeholder="0.00">
                                </div>
                            @endforeach
                        </div>
                        <!-- List Price Row -->
                        <div class="flex gap-3">
                            @foreach(config('payment.currencies') as $code => $currency)
                                ...
                                <div class="flex-shrink-0 w-32">
                                    ...
                                    <input type="number" name="{{ strtolower($code) }}_list_price" value="{{ old(strtolower($code) . '_list_price', $pricing->list_price ?? ($code === 'USD' ? $digitalAsset->list_price : '')) }}" step="0.01" min="0" class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded" placeholder="0.00">
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t">
                ...
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Update Asset</button>
            </div>
        </div>
    </form>
</div>
